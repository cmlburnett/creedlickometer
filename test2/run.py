...
def allfiles():
	...
	dt = datetime.datetime(2024,7,18, 11,0,0)
	v.AddMeasurement(dt, 1, 7.5, 0.0)
	v.AddMeasurement(dt, 3, 5.0, 10.0)
	v.AddMeasurement(dt, 5, 4.5, 11.5)
	v.AddMeasurement(dt, 6, 8.0, 8.0)
	v.AddMeasurement(dt, 7, 8.0, 12.5)
	v.AddMeasurement(dt, 8, 6.25, 12.0)
	v.AddMeasurement(dt, 9, 7.0, 9.5)
	v.AddMeasurement(dt, 12,13.0, 8.5)
	v.AddFill(dt, 1, None, 13.0)
	v.AddFill(dt, 3, 13.0, None)
	v.AddFill(dt, 5, 13.0, None)
	v.AddFill(dt, 8, 12.5, None)

	dt = datetime.datetime(2024,7,19, 11,0,0)
	v.AddMeasurement(dt, 1, 5.25, 0.0)
	v.AddMeasurement(dt, 3, 10.75, 10.0)
	v.AddMeasurement(dt, 5, 10.0, 10.75)
	v.AddMeasurement(dt, 6, 5.5, 7.5)
	v.AddMeasurement(dt, 7, 6.5, 12.5)
	v.AddMeasurement(dt, 8, 10.0, 12.0)
	v.AddMeasurement(dt, 9, 1.5, 9.5)
	v.AddMeasurement(dt, 12,12.5, 4.0)
